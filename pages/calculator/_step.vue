<template>
  <div class="calc_step flex space_around" v-if="!this.$apollo.loading">
    <div v-for="(question, id) in quizz" :key="'q' + id" v-if="question.step == currentStep"
      ...
      <div class="c_body px-2">
        <div class="answers flex wrap align_center space_between">
          <radio-ans @click.native="question.results.selectedOption = option.id" v-for="(option, index) in question.options"
            :key="'o' + index" :content="option" :selected="question.results.selectedOption == option.id"></radio-ans>
          <markAns v-for="(mark, index) in question.marks" :key="'m' + index" :content="mark"
            :value="markExist(question.id, mark.id)" @valueChange="markChanged(question.id, mark.id, $event)"></markAns>
          <input-ans v-for="(input, index) in question.inputs" :key="'i' + index"
            @input="inputChanged(question.id, input.id, $event)" :iType="input.type"
            :value="inputExist(question.id, input.id)" data-field="input" class="my-2 w-100" :label="input.label"
            :required="input.required" :placeholder="input.placeholder" :error="errors[input.name]"
            @errorField="errors[input.name] = $event;" :ref="input.name"></input-ans>
        </div>
      ...
      <div class="c_footer">
        ...
        <div class="flex align_center" :class="{ space_between: question.previous, end: !question.previous }">
          <fBtn v-if="question.previous" @click.native="previousQuestion(question)"
            class="pa-3 bg_lightGrey flex align_center center border f_link text_white mx-2">
            <h3 v-tr class="text_text1">Back</h3>
          </fBtn>
          <div class="flex align_center">
            <fBtn @click.native="skipQuestion(question)" v-if="question.next"
              class="pa-3 bg_lightGrey flex align_center center border f_link text_white mx-2">
              <h3 v-tr class="text_text1">skip</h3>
            </fBtn>
            <fBtn class="pa-3 bg_primary flex align_center center border f_link text_white mx-2"
              @click.native="nextQuestion(question)" v-if="question.next">
              <h3 v-tr>Processed</h3>
            </fBtn>
            <fBtn v-else @click.native="sendDetails(question)"
    ...
    <div class="w-35 bg_white pa-4 flex column space_between">
      ...
      <div class="c_body py-3 pt-5 flex2">
        <h4 v-for="(item, index) in cart.items" :key="index" class="flex align_center space_between text_text3 my-3">
          <span>{{ item.text }}</span>
...
export default {
  ...
  mounted() {
    console.log(this.quizzes, this.results);
