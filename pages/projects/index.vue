  ...
  <div v-if="!this.$apollo.loading">
    ...
    <section class="projects_list bg_white py-5">
      <div class="container">
        ...
        <div class="c_body flex wrap fill_width space_between" v-if="projects">
          <div :style="{ backgroundImage: `url(${api_url + getCoverImage(index)})` }"
            class="project_case elevate_1 pointer pair relative my-1" v-for="(item, index) in projects.length"
            :key="index" :class="{ pair: parinage[index] == 0, impair: parinage[index] == 1, 'mr-2': index % 2 == 0 }">
            <div class="overlay pa-4 flex column space_between fill_height">
              <div class="c_head">
                ...
                <h3 class="text_white">
                  <span class="mr-2" v-for="(industry, index) in deepFind(projects, `${index}.header.industries`)"
                    :key="index">{{ industry.name }}{{ index == deepFind(projects, `${index}.header.industries.length`) -
                      1 ? '' : ',' }}</span>
                </h3>
              </div>
              <div class="c_footer">
                <router-link tag="h3" :to="localePath(`/projects/${deepFind(projects, `${index}.slug`)}`)"
                  class="text_white txt_center">{{ $t('samples.projectList.view') }}</router-link>
              </div>
            </div>
          </div>
        </div>
...
export default {
  apollo: {
    projects: {
      ...
      query: projectQuery,
      variables() {
        return { locale: 'en' }; //*** this.$i18n.locale*/
      },
